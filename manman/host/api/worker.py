...
# async def server_create(instance: GameServerInstance) -> GameServerInstance:
#     with get_session() as sess:
#         # TODO - move check that it's not already dead to trigger
#         # DB is right place to do that, but doing this so I can learn
#         stmt = sqlalchemy.select(GameServerInstance).where(GameServerInstance.game_server_instance_id == instance.game_server_instance_id)
