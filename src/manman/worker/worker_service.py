...
class WorkerService(ManManService):
    ...
    def _shutdown(self):
        ...
        logger.info("Initiating shutdown cascade to %d dependent servers", len(self._servers))
        ...
        for server in servers_to_shutdown:
            ...
            timeout_seconds = 30
            wait_count = 0
            while not server.is_shutdown and wait_count < timeout_seconds * 10:  # Check every 100ms
                time.sleep(0.1)
                wait_count += 1
            ...
            if server.is_shutdown:
                ...
                logger.warning("Server %s shutdown timeout after %d seconds", server.instance, timeout_seconds)
        
        logger.info("All dependent servers shutdown complete, proceeding with worker shutdown")
