...
class ManManConfig:
    ...
    def get_api_config(cls, api_name: str) -> APIConfig:
        ...
        if api_name not in cls.API_CONFIGS:
            raise ValueError(f"Unknown API name: {api_name}. Valid options are: {', '.join(cls.KNOWN_API_NAMES)}")
        return cls.API_CONFIGS[api_name]
    
    @classmethod
    def validate_api_name(cls, api_name: str) -> Literal["experience-api", "status-api", "worker-dal-api"]:
