...
class TestWorkerShutdownCascade(unittest.TestCase):
    ...
    def test_shutdown_with_no_servers(self, mock_api_client_class, mock_auth_client):
        ...
        self.mock_wapi.worker_shutdown.assert_called_once_with(self.mock_worker_instance)

    @patch("manman.worker.worker_service.get_auth_api_client")
    @patch("manman.worker.worker_service.WorkerAPIClient")
    def test_shutdown_cascade_to_dependent_servers(self, mock_api_client_class, mock_auth_client):
    ...
    @patch("manman.worker.worker_service.get_auth_api_client")
    @patch("manman.worker.worker_service.WorkerAPIClient")
    def test_shutdown_waits_for_server_completion(self, mock_api_client_class, mock_auth_client):
        ...
        self.assertGreaterEqual(end_time - start_time, 0.2)  # Should have waited at least 200ms
    ...
    @patch("manman.worker.worker_service.get_auth_api_client")
    @patch("manman.worker.worker_service.WorkerAPIClient")
    def test_shutdown_handles_already_shutdown_servers(self, mock_api_client_class, mock_auth_client):
